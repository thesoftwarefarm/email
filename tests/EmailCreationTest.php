...
class EmailCreationTest extends TestCase
{
    ...
    public function test_email_is_saved_in_database()
    {
        $email = (new Email())
            ...
            ->subject('Subject')
            ->body('Body')
            ->addAttachment(new Attachment('attachment_1.txt'))
            ->addAttachment(new Attachment('attachment_2.txt', 'custom_name_2.txt'))
            ->addAttachment(new Attachment('attachment_3.txt', 'custom_name_3.txt', 's3'))
